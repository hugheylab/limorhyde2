...
Here we show two options for using `limorhyde2` to analyze RNA-seq data: [DESeq2](https://bioconductor.org/packages/release/bioc/html/DESeq2.html) and [limma-voom](https://bioconductor.org/packages/release/bioc/html/limma.html). In general, the two approaches give very similar results. We slightly prefer DESeq2, as we have observed that it seems less susceptible to overestimating rhythm amplitudes for genes that have zero counts in some samples.
