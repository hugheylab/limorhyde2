...
test_that('getSm returns a data.table with specific columns', {
  ...
  mdNum = data.table(time = samples, sample_id = paste0("sample_", 1:length(samples)),
                     ...
                     misc = c('test'))
  d = getSm(mdNum, 'time', 'conds')


  expect_error(getSm(mdLetter, 'timed', NULL))
  expect_error(getSm(mdLetter, 1, NULL))
  expect_error(getSm(mdLetter, c('timed', 'd'), NULL))
  expect_error(getSm(mdLetter, 'time', NULL))
  expect_error(getSm(mdNum, 'time', 'conditions'))

  expect_s3_class(getSm(mdNum, 'time', NULL), 'data.table')
